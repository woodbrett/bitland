...
def queueNewBlockFromPeer(block_height,block,peer='', use_threading=True):
    ...
    if use_threading == True:    
        t5 = threading.Thread(target=analyzeNewBlockFromPeer,args=(block_height,block,b'',peer,),daemon=True)
        t5.start()
...
def queueNewTransactionFromPeer(transaction_hex,use_threading=True,peer=''):
    ...
    if use_threading == True:
        t4 = threading.Thread(target=analyzeNewTransactionFromPeer,args=(transaction_hex,peer,),daemon=True)
        t4.start()
        #t4.join() this t4.join may cause circular logic

    else:
        analyzeNewTransactionFromPeer(transaction_hex,peer)

    return None


def analyzeNewTransactionFromPeer(transaction_hex,peer=''):
    
    transaction_bytes = unhexlify(transaction_hex)
    
    validate_transaction = validateAddTransactionMempool(transaction_bytes, use_queue=True)
